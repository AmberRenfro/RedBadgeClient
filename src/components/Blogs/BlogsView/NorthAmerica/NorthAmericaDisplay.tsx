...
interface NorthAmericaDisplayProps {
  blogs: [BlogJSON];
...
interface NorthAmericaDisplayState {
  singleBlog: BlogJSON;
...
class NorthAmericaDisplay extends React.Component<
  NorthAmericaDisplayProps,
  NorthAmericaDisplayState,
  BlogJSON
> {
  constructor(props: NorthAmericaDisplayProps) {
    super(props);
    this.state = {
      singleBlog: {
        id: 0,
  ...
  componentDidUpdate(
    prevProps: Readonly<NorthAmericaDisplayProps>,
    prevState: Readonly<NorthAmericaDisplayState>
  ) {
  ...
  NorthAmericaMapper = () => {
    return this.props.blogs.map((blog: BlogJSON, index) => {
      if (blog.destination === "North America") {
        return (
          <Col key={index}>
...
export default NorthAmericaDisplay;
