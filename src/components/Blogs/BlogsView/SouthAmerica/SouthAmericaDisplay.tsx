...
interface SouthAmericaDisplayProps {
  blogs: [BlogJSON];
...
interface SouthAmericaDisplayState {
  singleBlog: BlogJSON;
...
class SouthAmericaDisplay extends React.Component<
  SouthAmericaDisplayProps,
  SouthAmericaDisplayState,
  BlogJSON
> {
  constructor(props: SouthAmericaDisplayProps) {
    super(props);
    this.state = {
      singleBlog: {
        id: 0,
  ...
  componentDidUpdate(
    prevProps: Readonly<SouthAmericaDisplayProps>,
    prevState: Readonly<SouthAmericaDisplayState>
  ) {
  ...
  SouthAmericaMapper = () => {
    return this.props.blogs.map((blog: BlogJSON, index) => {
      if (blog.destination === "South America") {
        return (
          <Col key={index}>
...
export default SouthAmericaDisplay;
